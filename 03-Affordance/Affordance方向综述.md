...
大模型时代 (2024-2025): VLM 驱动的 affordance
├─ 2024: ManiGaussian → 3DGS + affordance (动态场景)
├─ 2024: Robo-ABC → 大规模 affordance 数据自动生成
├─ 2024: InstructGrasp → 语言引导的 affordance 理解
├─ 2024: GraspGPT → LLM + VLM 联合 affordance 推理
├─ 2025: ReKep → 关系关键点约束 → affordance 的结构化表示
├─ 2025: GenAug → 生成式数据增强 → affordance 的 scaling
...
│   代表: InstructGrasp, GraspGPT, ManipVQA
...
| 维度 | Where2Act | AnyGrasp | ManiGaussian | GraspGPT | ReKep |
|------|-----------|----------|-------------|----------|-------|
| **年份** | 2021 | 2023 | 2024 | 2024 | 2025 |
| **输入** | 部分点云 | 点云 | 多视角 RGB-D | RGB + 文本 | RGB-D |
| **输出** | 逐点 (推/拉/抓) | 6-DoF 抓取位姿 | 3D affordance + 操作轨迹 | 抓取位姿 + 文本解释 | 关系关键点约束 |
| **动作类型** | 推/拉/抓 | 抓取 | 抓/推/按/开 | 抓取 | 任意 (约束定义) |
| **泛化** | 同类别物体 | 开放世界物体 | 动态场景 | 语言引导零样本 | 结构化泛化 |
| **推理速度** | 快 | 实时 (>30fps) | 慢 (3DGS) | 中等 | 快 |
| **开源性** | ✅ | ✅ | ✅ | ❌ | ✅ |
| **核心创新** | 像素级 affordance | 7-DoF 通用 | 3DGS 动态 | LLM 常识 | 约束数学化 |
...
- 🔥 **语言引导 affordance**（GraspGPT/InstructGrasp）："抓最左边的杯子" → 语言+affordance 联合
...
2. **从静态到动态**：affordance 随物体状态变化（门开着/关着 → 可做的动作不同）
3. **从单点到关系**：多个物体之间的 affordance 关系（"锤子可以用来敲这个钉子" → 跨物体 affordance）
...
| **长周期 affordance** | 当前只看"下一步能做什么"，不看"做完后还能做什么" | 分层 affordance, 任务链预测 |
...
| **Dex-Net 4.0** | 2020 | 大规模分析抓取的经典 | 1.5h |
...
| **ReKep** | 2025 | 关系关键点 → structured affordance | 2h |

### 前沿级
| 论文 | 年份 | 一句话 | 阅读时间 |
|------|------|--------|---------|
| **GraspGPT** | 2024 | LLM 常识 + affordance 联合推理 | 2.5h |
| **Robo-ABC** | 2025 | 大规模 affordance 数据自动生成 | 3h |
...
- Day 4-5: 对比几何方法和学习方法的核心差异
- Day 6-7: 在 robosuite 中运行 affordance 预测 + 抓取 pipeline

### 第 3 周：跟上大模型时代
- Day 1-3: 读 GraspGPT/InstructGrasp，理解语言与 affordance 的结合
