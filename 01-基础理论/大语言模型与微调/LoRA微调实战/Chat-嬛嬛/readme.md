...
我们选择 `LLaMA3_1-8B-Instruct` 模型进行微调，首先下载模型，创建一个`model_download.py`文件，输入以下内容：
...
其次，准备训练代码。在当前目录下放置`train.py`，修改其中的数据集路径和模型路径即可。

> *也可以使用 HuggingFace PEFT 库中的 Lora 微调教程。*
...
Chat-嬛嬛 是一个角色扮演 LoRA 微调的完整案例——从数据提取、格式转换到模型训练和推理。掌握了这个流程，你可以用同样的方法为任何小说/剧本角色打造个性化 AI。
